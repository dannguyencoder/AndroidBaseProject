package com.example.vinhcrazyyyy.androidbaseproject.adapter_views.endless_scrolling.progress_bar;
...
/*To display the last row as a ProgressBar indicating that the ListView is loading data,
we do the trick in the Adapter. Having defined two types of views
in getItemViewType(int position), we can display the last row differently
from a normal data row. It can be a ProgressBar or some text to indicate that the ListView
has reached the last row by comparing the size of data List to the number of items
on the server side.*/
...
*
* This parent Adapter takes care of displaying ProgressBar in a row
* or indicating that it has reached the last row*/
public abstract class ProgressBarAdapter<T> extends BaseAdapter {
    ...
    public ProgressBarAdapter(List<T> dataList, Activity mActivity) {
    ...
    /*The size of the List plus one, the one is the last row
    ...
    * the last row indicates the user that the ListView is loading more data*/
    ...
    /**
     * A subclass should overide this method to supply the data row.
     * @param position
     * @param converView
     * @param parent
     * @return
    ...
    /**
     * return a View to be displayed in the last row
     * @param position
     * @param convertView
     * @param parent
     * @return
    ...
    public View getFooterView(int position, View convertView, ViewGroup parent) {
        ...
        if (row == null) {
            row = mActivity.getLayoutInflater().inflate(R.layout.progress, parent, null);
        }
